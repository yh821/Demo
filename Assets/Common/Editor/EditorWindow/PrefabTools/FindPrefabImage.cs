...
namespace Common.Editor
{
	...
	{
		public FindPrefabImage(PrefabTools window) : base(window) { }
		...
		public override void OnGUI()
		{
			...
			{
				mInputTexture =
					EditorGUILayout.ObjectField("查找的图片", mInputTexture, typeof(Texture2D), false,
			...
			EditorGUILayout.BeginHorizontal();
			{
				...
				if (GUILayout.Button("开始查找"))
				{
					if ((mIsSprite && mInputSprite != null) || (!mIsSprite && mInputTexture != null))
						...
						win.ShowNotification(new GUIContent("请设置查找的图片"));
				}
			}
			EditorGUILayout.EndHorizontal();
				...
				EditorGUILayout.BeginHorizontal();
				{
					...
					if (GUILayout.Button("克隆/删除", GUILayout.MaxWidth(64)))
					{
						...
						if (prefab != null)
						{
							...
							var target = win.GetSceneObject(fileName);
							if (target == null)
							{
								var go = PrefabUtility.InstantiatePrefab(prefab) as GameObject;
								go.transform.SetParent(win.TempParent, false);
								go.name = fileName;
							}
							else
							{
		...
		protected override bool FindChildrenPath(GameObject prefab, ref List<string> paths)
		{
			...
			foreach (var rawImage in mPrefabChildren)
				paths.Add(EditorHelper.GetPath(rawImage));
			...
			if (mIsSprite)
			{
				...
				{
					if (image.sprite != null && image.sprite == mInputSprite &&
					    !mPrefabChildren.Contains(image.gameObject))
						mPrefabChildren.Add(image.gameObject);
				}

				foreach (var image in mPrefabChildren)
					paths.Add(EditorHelper.GetPath(image));
			}
