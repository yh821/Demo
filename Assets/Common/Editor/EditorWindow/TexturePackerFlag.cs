...
namespace Common.Editor
{
	public class TexturePackerFlag : EditorWindow
	{
		...
		void OnGUI()
		{
			...
			addTexture =
				EditorGUILayout.ObjectField("", addTexture, typeof(Texture2D), false, GUILayout.MaxWidth(64)) as
					Texture2D;
		...
		void DrawTextureItem(Data data, int searchIndex)
		{
			...
			EditorGUILayout.LabelField(string.Format("{0}: {1} x {2}", data.name, data.texture.width,
				data.texture.height));
