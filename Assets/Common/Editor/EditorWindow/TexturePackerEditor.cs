...
namespace Common.Editor
{
	public class TexturePackerEditor : EditorWindow
	{
		...
		private void OnGUI()
		{
			...
			inputTexture =
				(Texture2D) EditorGUILayout.ObjectField("", inputTexture, typeof(Texture2D), false,
			...
			if (!string.IsNullOrEmpty(particlePath))
			{
				...
				{
					SVNHelper.SvnCommand(SVNHelper.Command.Update, particlePath);
				}
			}
			...
			if (!string.IsNullOrEmpty(particlePath))
			{
				...
				{
					SVNHelper.SvnCommand(SVNHelper.Command.Commit, particlePath);
				}
			}
			...
			if (inputTexture != null && Directory.Exists(particlePath))
			{
				...
				mMaxSize = Mathf.Clamp(EditorGUILayout.IntField("MaxSize:", mMaxSize, GUILayout.MaxWidth(100)), 2,
					4096);
				...
				{
					var altasPath = Path.Combine(IOHelper.ProjPath,
