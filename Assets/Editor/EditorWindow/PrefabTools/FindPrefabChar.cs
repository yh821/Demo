...
{
	public class FindPrefabChar : PageBase
	{
		...
		public FindPrefabChar(PrefabTools window)
		{
			win = window;
		...
		public void OnGUI()
		{
			...
			if (dict.Count > 0)
			{
				...
				for (int i = 0; i < searchData.Count; i++)
				{
					...
					if (charData.foldout)
					{
						foreach (var assetPath in charData.assetPaths)
						{
							EditorGUILayout.BeginHorizontal();
							...
							if (GUILayout.Button("显示/移除", GUILayout.MaxWidth(64)))
							{
								...
								if (stage == null || stage.prefabAssetPath != assetPath)
									AssetDatabase.OpenAsset(AssetDatabase.LoadMainAssetAtPath(assetPath));
								else StageUtility.GoToMainStage();
#else
								...
								if (asset != null)
								{
									...
									if (target == null)
									{
										...
										if (!isHaveCanvas && win.TempParent != null)
											selectPrefab.transform.SetParent(win.TempParent, false);
		...
		private void RefreshSearchText()
		{
			...
			else
			{
				foreach (var chn in dict.Values)
				{
					if (chn.cn_text.IndexOf(searchKey, StringComparison.Ordinal) != -1)
						searchData.Add(chn);
				}
			}
		}
		...
		{
			var filename = Path.Combine(IOHelper.ProjPath, "预设文字.csv");
		...
		{
			hasCanvas = false;
			if (win.TempParent != null)
			{
				var t = win.TempParent.Find(name);
				if (t != null)
					return t.gameObject;
			}
