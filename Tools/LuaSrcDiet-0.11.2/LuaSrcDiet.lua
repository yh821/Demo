...
local TTYPES = {
	TOK.KEYWORD, TOK.NAME, TOK.NUMBER, -- grammar
	TOK.STRING, TOK.LSTRING, TOK.OP,
	TOK.EOS,
	TOK.COMMENT, TOK.LCOMMENT, -- non-grammar
	TOK.EOL, TOK.SPACE,
}
...
local function process_file(srcfl, destfl)
	...
	toklist, seminfolist, toklnlist = optlex.optimize(option, toklist, seminfolist, toklnlist)
...
local function main()
	...
	while i <= argn do
		...
		if dash == "-" then
			...
			elseif o == "-s" then
				if not p then
					die("-s option needs suffix specification")
				end
				suffix = p
				i = i + 1
			elseif o == "-o" then
				if not p then
					die("-o option needs a file name")
				end
		...
		elseif dash == "--" then
			...
			elseif o == "--keep" then
				if not p then
					die("--keep option needs a string to match for")
				end
				option.KEEP = p
				i = i + 1
			elseif o == "--plugin" then
				if not p then
					die("--plugin option needs a module name")
				end
				if option.PLUGIN then
					die("only one plugin can be specified")
				end
				option.PLUGIN = p
			...
			elseif o == "--details" then
				option.DETAILS = true
			elseif o == "--params" then
				option.PARAMS = option.PARAMS or {}
				table.insert(option.PARAMS, p)
				i = i + 1
			elseif OPTION[o] then
				-- lookup optimization options
				set_options(o)
			else
				die("unrecognized option " .. o)
			end
